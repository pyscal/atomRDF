...
def visualize_sample(g,
    ...
    #first step is to create the json file
    # we can start with a dict
    sample_id = get_title_from_BNode(sample)
    gdict = {}
    gdict["nodes"] = []
    gdict["edges"] = []
    start_plot = False
    ...
    for k in g:
        #check if the current item is a sample
        if parse_object(k[0])[1] == "BNode":
            if parse_object(k[2])[1] == "URIRef":
                if get_string_from_URI(k[2]) == 'cmso.AtomicScaleSample':
                    #now check if sample id matches given id
                    #print(get_title_from_BNode(k[0]), sample_id)
                    if get_title_from_BNode(k[0]) == sample_id:
                        #start plotting
                        start_plot = True
                    else:
                        start_plot = False
